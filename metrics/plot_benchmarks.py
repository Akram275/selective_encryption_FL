...
def plot_round_metrics(round_rows, output_dir):
    ...
    axes[1, 1].plot(rounds, plaintext_kb, label="Plaintext baseline", color="tab:gray")
    #axes[1, 1].plot(rounds, encrypted_kb, marker="s", label="Encrypted slice", color="tab:brown")
    axes[1, 1].plot(rounds, encrypted_kb, label="Encrypted slice", color="tab:brown")
    #axes[1, 1].plot(rounds, dp_kb, marker="^", label="DP slice", color="tab:olive")
    axes[1, 1].plot(rounds, dp_kb, label="DP slice", color="tab:olive")
    #axes[1, 1].plot(rounds, hybrid_kb, marker="d", label="Hybrid total", color="tab:cyan")
    axes[1, 1].plot(rounds, hybrid_kb, label="Hybrid total", color="tab:cyan")
    ...
    ax_fraction.plot(rounds, fhe_fraction, linestyle="--", color="tab:orange", label="FHE fraction")
...
def plot_client_metrics(client_rows, output_dir):
    ...
    for client_id in client_ids:
        ...
        axes[0].plot(rounds, train_time, label=f"Client {client_id}")
        axes[1].plot(rounds, fhe_coordinates, label=f"Client {client_id} FHE coords")
        axes[1].plot(rounds, hybrid_kb, linestyle="--", label=f"Client {client_id} payload KB")
